...
export default function ScoreCard({currentBatting, strike, bowler, inning, currentInning, teamName, totalScoreofInning}) {
  ...
  if(currentBatting.length>0 ){
    ...
    batsman1=currentBatting[0].name;
    name1=batsman1;
    if(width < 900)batsman1=batsman1.substring(0,5);
    batsman1 = <>{batsman1}&ensp;{currentBatting[0].run}</>;
  }
  if(currentBatting.length>1){
    index2=currentBatting[1].index;
    batsman2=currentBatting[1].name;
    name2=batsman2;
    if(width < 900)batsman2=batsman2.substring(0,5);
    batsman2 = <>{batsman2}&ensp;{currentBatting[1].run}</>;
  }
  let bowlerName ="", bowlerFeild ="";
  if(bowler.index>-1){
    bowlerName = bowler.name;
    if(width < 900)bowlerName=bowlerName.substring(0,8);
    bowlerFeild = <>{bowlerName}&ensp;{bowler.run}</>;
  }
  return (
    <div className = "scoreBoard">
      <div>
        <span datatooltip={teamName[currentInning]} className = "score">Team{currentInning+1}&ensp;{totalScoreofInning}</span>
